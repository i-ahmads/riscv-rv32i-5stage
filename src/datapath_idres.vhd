...
end entity datapath_idres;

architecture rtl of datapath_idres is
  ...
  -- ID-stage branch/jump resolution (this pipeline's distinguishing feature)
  signal id_fwd_a, id_fwd_b : std_logic;
  signal id_cmp_a, id_cmp_b : std_logic_vector(31 downto 0);
  signal id_branch_cond, id_branch_taken, id_needs_cmp : std_logic;
  signal id_pc_plus_imm, id_jalr_target, id_target : std_logic_vector(31 downto 0);

begin

  ----------------------------------------------------------------- IF stage
  pc_plus4 <= std_logic_vector(unsigned(pc_current) + 4);
  pc_next  <= id_target when id_branch_taken = '1' else pc_plus4;
  ...
  u_id_forward : entity work.id_forward_unit
    port map (if_id_rs1_addr => id_rs1_addr, if_id_rs2_addr => id_rs2_addr,
              ex_mem_rd_addr => exmem_rd_addr, ex_mem_reg_write => exmem_reg_write,
              fwd_a => id_fwd_a, fwd_b => id_fwd_b);

  -- MEM/WB source is already covered by register_file's write-through
  -- bypass, so only the EX/MEM case needs an explicit mux here
  id_cmp_a <= exmem_fwd_value when id_fwd_a = '1' else id_rs1_data;
  id_cmp_b <= exmem_fwd_value when id_fwd_b = '1' else id_rs2_data;

  u_branch_unit : entity work.branch_unit
    port map (a => id_cmp_a, b => id_cmp_b, funct3 => id_funct3, taken => id_branch_cond);

  id_pc_plus_imm <= std_logic_vector(unsigned(ifid_pc) + unsigned(id_imm));
  id_jalr_target <= std_logic_vector(unsigned(id_cmp_a) + unsigned(id_imm));
  -- (LSB cleared per spec, applied where id_target is consumed below)
  id_needs_cmp    <= id_branch or id_jump or id_is_jalr;
  id_target       <= (id_jalr_target(31 downto 1) & '0') when id_is_jalr = '1' else id_pc_plus_imm;
  id_branch_taken <= (id_branch and id_branch_cond) or id_jump or id_is_jalr;

  u_hazard_detection : entity work.hazard_detection_unit_idres
    port map (id_ex_reg_write => idex_reg_write, id_ex_rd_addr => idex_rd_addr,
              if_id_rs1_addr => id_rs1_addr, if_id_rs2_addr => id_rs2_addr,
              if_id_needs_cmp => id_needs_cmp, load_use_mem_read => idex_mem_read,
  ...
  -- (No EX-stage branch/jump target logic here: this pipeline resolves
  -- branches and jumps in ID instead - see the ID stage section above.)
